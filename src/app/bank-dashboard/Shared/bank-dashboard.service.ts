...
baseURL:string='http://elitecore.cloudapp.net:8081';
  ...
  async GetRateOfInterestOfBank(){
    await this.http.get(this.baseURL+'/api/BankDashBoard/GetRateOfInterestOfBank?userId='+this.userId).map((data:Response) => {return data.json() as RateOfInterestOfBankModel[];
  }).toPromise().then(x=>{
  ...
  UpdateRateOfInterest(rateModel: RateOfInterestOfBankModel){
    ...
    return this.http.post(this.baseURL+'/api/BankDashBoard/UpdateRateOfInterestOfBank',body,requestOptions).map(x=> x.json());
  }
  ...
  PublishAndUnPublish(IsPublished:boolean){
    ...
    var requestOptions=new RequestOptions({method:RequestMethod.Post,headers:headerOptions});
    return this.http.post(this.baseURL+'/api/BankDashBoard/PublishAndUnPublish?IsPublished='+IsPublished+"&userId="+this.userId,requestOptions).map(x=> x.json());
  }

  async GetUserGroupForSettingRateOfInterestVisibility(){
    await this.http.get(this.baseURL+'/api/BankDashBoard/GetUserGroupForSettingRateOfInterestVisibility?userId='+this.userId).map((data:Response) => {return data.json() as GroupModel[];
  }).toPromise().then(x=>{
